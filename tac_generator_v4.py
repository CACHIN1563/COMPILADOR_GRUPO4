# =================================================================
# GENERADOR DE CÓDIGO INTERMEDIO (TAC) - PROYECTO FINAL (V4)
# =================================================================

from gramatica_v4Visitor import gramatica_v4Visitor

class TACGeneratorV4(gramatica_v4Visitor):
    ...
    def visitDeclStructG4(self, ctx):
        # En TAC simple, declaracion de struct no genera codigo ejecutable.
        return None
    ...
    def visitVarDeclarationG4(self, ctx):
        ...
        if ctx.expr():
            ...
            self.emit(f"{nombre} = 0") # default init
    ...
    def visitExprCastingG4(self, ctx):
        ...
        val = self.visit(ctx.expr())
        temp = self.new_temp()
        self.emit(f"{temp} = ({tipo}) {val}")
        return temp

    def visitExprTernarioG4(self, ctx):
        cond = self.visit(ctx.expr(0))
        val_true = ctx.expr(1)
        val_false = ctx.expr(2)
        
        label_false = self.new_label()
        label_end = self.new_label()
        temp = self.new_temp()
        
        self.emit(f"if not {cond} goto {label_false}")
        t_val = self.visit(val_true)
        self.emit(f"{temp} = {t_val}")
        self.emit(f"goto {label_end}")
        
        self.emit(f"{label_false}:")
        f_val = self.visit(val_false)
        self.emit(f"{temp} = {f_val}")
    ...
    def get_lvalue_text(self, lvalue_ctx):
        ids = lvalue_ctx.TK_ID()
        return ".".join([id.getText() for id in ids])

    def visitAsignacionCoreG4(self, ctx):
        nombre = self.get_lvalue_text(ctx.lvalue())
        val = self.visit(ctx.expr())
        self.emit(f"{nombre} = {val}")
        return nombre
    ...
    def visitSentenciaSwitchG4(self, ctx):
        cond = self.visit(ctx.switchStatement().expr())
        end_label = self.new_label()
        
        self.loop_stack.append((None, end_label)) # para soportar break dentro del switch
        ...
        for case in ctx.switchStatement().caseStatement():
            ...
            self.emit(f"if {cond} == {case_val} goto {label}")
    ...
    def visitSentenciaContinueG4(self, ctx):
        if self.loop_stack:
            ...
            if start_label: # en switch, start_label es None
